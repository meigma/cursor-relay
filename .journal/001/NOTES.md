...
## 2026-08-14 08:59 — Social-agent deployment changes choice
The target is an always-on Hermes social agent on Buzz rather than a local coding-agent delegation. This reverses the transport choice: the SDK Bridge better matches hosted application lifecycle, durable Agent/Run state, structured usage, cancellation, cloud optionality, and tool callbacks; ACP's local CLI login, interactive approval protocol, and editor-oriented stdio lifecycle become liabilities.
Avoid nesting the Cursor SDK agent inside Hermes's model/tool loop. Prefer Cursor as the reasoning agent and Hermes as the messaging gateway: consume Hermes conversation events, map each Buzz conversation to a durable Cursor agent, submit the inbound message through the SDK, and send the result through Hermes. Hermes's MCP server already exposes event polling, history, attachments, and message sending, and can be attached to the Cursor agent when social tools are needed.
The remaining product decision is credential ownership. A user API key consumes that user's Cursor plan but must be delegated securely to the hosted service; a service-account key bills the operator's team. There is no hosted use of an individual's subscription without some user credential delegation.

## 2026-08-14 09:25 — SDK Bridge spike succeeded
Created disposable branch `spike/cursor-sdk-hermes` at `.wt/spike-cursor-sdk-hermes` with a top-level `spike/` Go command. It pins the experiment to SDK Bridge v1.0.28 (darwin-arm64 archive SHA-256 `52ebfdab4e7806270122bea6c8f972646516297343c483e6700b37d444515af5`), manages the bridge ready-line/token/shutdown lifecycle, sends Connect JSON streams, creates and resumes tool-free local agents, persists Hermes session-to-agent mappings, and accepts the current Hermes `events_poll` message shape.
Live verification used a temporary 15-minute user key minted by the first-party SDK and revoked with `Cursor.auth.logout`; `~/.cursor/sdk` was empty afterward. Two user events in one Hermes session reused one Cursor agent and remembered `indigo`; an intervening assistant event produced no response. A fresh bridge process resumed the same agent ID and again recalled `indigo`. Every result included run ID, duration, and token usage.
Material learning: even minimal social turns carried roughly 3.3–3.7k input tokens, so agent context overhead may dominate social-message cost. Local RunResult reports tokens but the bridge's billed-cost API is cloud-only. The spike does not run a real Hermes/Buzz gateway or call `messages_send`; it also omits cancellation, durable event-cursor idempotency, concurrent conversations, dropped-stream recovery, social tool callbacks, and production credential storage. Production code should use generated `sdk.v1`/Connect clients instead of the spike's hand-written JSON subset.

## 2026-08-14 10:25 — Economics accepted
The user confirms from direct experience that Cursor's own-model subscription economics are at least roughly 20x better than paying street per-token rates. Treat the observed 3.3–3.7k input-token overhead as capacity and latency data, not a reason to reject the SDK path on cost. Remaining spike questions are subscription throughput and throttling under social concurrency, credential custody, context growth/compaction, and whether Hermes persona and memory can be preserved when Cursor owns the reasoning loop.
